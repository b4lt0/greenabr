...
SCHEMES = ['GreenABR', 'Pensieve', 'BB', 'Bolae', 'DD', 'Dynamic', 'Throughput', 'model']
...
def arrowed_spines(fig, ax):
    ...
    # draw x and y axis
    ax.arrow(xmin, 0, xmax - xmin, 0., fc='k', ec='k', lw=lw, head_width=hw, head_length=hl, overhang=ohg,
             length_includes_head=True, clip_on=False)
    ax.arrow(xmin, ymin, 0., ymax - ymin, fc='k', ec='k', lw=lw, head_width=yhw, head_length=yhl, overhang=ohg,
             length_includes_head=True, clip_on=False)
...
def getAllResults(video, scheme):
    f_path = SUMMARY_DIR + video + '_' + scheme + '_summaryResults.csv'
    data = pd.read_csv(f_path)
    return np.reshape(np.array(data['Efficiency_Avg']), -1), np.reshape(np.array(data['Energy_Avg']), -1), np.reshape(
        np.array(data['Efficiency_X']), -1), np.reshape(np.array(data['Efficiency_G']), -1), np.reshape(
        np.array(data['QoE']), -1), np.reshape(np.array(data['Rebuffer_Time']), -1), \
...
def draw_all_normalized():
    ...
    for v in videos:
        ...
        for scheme in SCHEMES:
            ...
            graph_data = [totalEff_a, totalEn_a, totalEff_g, totalEff_x, totalQoE, rebufferTime, smoothChange,
                          total_energy_g, total_energy_x, dataUsage]
        ...
        for i, scheme in enumerate(SCHEMES):
            if i == 0:
                ...
                plt.bar(br, schemedata[scheme], bar_width, color=colors[i], edgecolor='w', hatch=patternstyles[i] * 3,
                        lw=1.)
        ...
        for i, scheme in enumerate(SCHEMES):
            ...
            brnew = [x + bar_width for x in br]
            br = [x for x in brnew]
        ax.legend(SCHEMEFULLNAMES, bbox_to_anchor=(0, 1.02, 1, 1), loc="lower left", ncol=len(SCHEMES) // 2 + 1,
                  mode="expand", prop=legend_font_dict, facecolor='white', framealpha=1)
        # Adding Xticks
...
def draw_all_normalized_avg():
    ...
    energy_x_n = 0
    for s in SCHEMES:
        eff_a[s], en_a[s], eff_x[s], eff_g[s], qoe[s], rebuf[s], smooth[s], data[s], energy_g[s], energy_x[
            s] = getAllResultsAvg(s)
    ...
    for scheme in SCHEMES:
        ...
        normalized_all.append(
            [scheme, totalEff_a, totalEn_a, totalEff_x, totalEff_g, totalQoE, rebufferTime, smoothChange,
             total_energy_g, total_energy_x, dataUsage])

        schemedata.setdefault(scheme, graph_data)

    groupnum = len(schemedata[SCHEMES[0]])
    normalized_all_data = pd.DataFrame(normalized_all,
                                       columns=['Scheme', 'Avg Efficiency', 'Avg Energy', 'Efficiency_X',
                                                'Efficiency_G', 'QoE', 'Rebuffer', 'Smooth', 'Energy_G', 'Energy_X',
                                                'Data'])
