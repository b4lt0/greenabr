...
def CreateSummaryResults(video, model):
    ...
    summary_data = pd.DataFrame(index=traces,
                                columns=['Efficiency_Avg', 'Efficiency_X', 'Efficiency_G', 'QoE', 'Bitrate',
                                         'Throughput', 'Buffer_Size', 'VMAF', 'Rebuffer_Time', 'Rebuffer_Count',
                                         'Smoothness_Change', 'Data', 'Energy_Avg', 'Energy_G', 'Energy_X'])
            ...
            with open(log_dir + l, 'r') as f:
                ...
                for line in f:
                    ...
                    if (float(parse[0]) > 0) & (float(parse[0]) < float(VIDEO_LEN)):
                        network_energy = Estimate_Network_Power_Consumption(thr, float(
                            parse[4]) / 1000000.0)  # network energy
                ...
                summary_data.loc[trace] = [eff_avg, qoe / np.sum(energy_x) * 1000, qoe / np.sum(energy_g) * 1000, qoe,
                                           np.sum(bit_rate), np.sum(throughput), np.sum(buffer_size), np.sum(vmaf),
                                           np.sum(rebuffer), rebuffer_c, np.sum(smooth), np.sum(data_size), en_avg,
                                           np.sum(energy_g), np.sum(energy_x)]
...
def createAvgFiles():
    videos = ['tos', 'bbb', 'doc']
    models = ['BB', 'Bolae', 'DD', 'Dynamic', 'Throughput', 'GreenABR', 'Pensieve', 'model']
    for m in models:
